...
exports.generatePDF = async (req, res) => {
  try {
    ...
    const bankDetails = invoice.bank_account_id || await BankDetails.findOne();
...
exports.viewPDF = async (req, res) => {
  try {
    ...
    if (!invoice.pdf_base64) {
      ...
      return res.status(200).json({ success: true, data: { pdf_base64: pdfBase64 } });
    }

    res.status(200).json({ success: true, data: { pdf_base64: invoice.pdf_base64 } });
  } catch (error) {
    res.status(400).json({ success: false, error: error.message });
  }
};
